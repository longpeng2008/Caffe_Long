...
class AdaptedWeightedSigmoidCrossEntropyLossLayerTest : public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

 protected:
  AdaptedWeightedSigmoidCrossEntropyLossLayerTest()
  ...
  virtual ~AdaptedWeightedSigmoidCrossEntropyLossLayerTest() {
    delete blob_bottom_data_;
    delete blob_bottom_targets_;
    delete blob_top_loss_;
  }

  Dtype AdaptedWeightedSigmoidCrossEntropyLossReference(const int count, const int num,
                                         const Dtype* input,
                                         const Dtype* target) {
    Dtype loss = 0;
  ...
  void TestForward() {
    ...
    for (int i = 0; i < 100; ++i) {
      ...
      AdaptedWeightedSigmoidCrossEntropyLossLayer<Dtype> layer(layer_param);
      ...
      Dtype reference_loss = kLossWeight * AdaptedWeightedSigmoidCrossEntropyLossReference(
...
TYPED_TEST_CASE(AdaptedWeightedSigmoidCrossEntropyLossLayerTest, TestDtypesAndDevices);

TYPED_TEST(AdaptedWeightedSigmoidCrossEntropyLossLayerTest, TestAdaptedWeightedSigmoidCrossEntropyLoss) {
  this->TestForward();
}

TYPED_TEST(AdaptedWeightedSigmoidCrossEntropyLossLayerTest, TestGradient) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  const Dtype kLossWeight = 3.7;
  layer_param.add_loss_weight(kLossWeight);
